...
describe("state reducer", () => {
  it("reconstructs state from event history", () => {
    ...
    const state = reduceEvents({ version: 1, id, status: "starting", turn: 0, lastCommandSeq: 0, lastEventSeq: 0 }, [
    ...
    expect(state).toMatchObject({ status: "completed", turn: 1, lastCommandSeq: 1, lastEventSeq: 4 });
  });
});
