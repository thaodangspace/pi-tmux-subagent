...
function setup(action: string, message = "do it") {
  const state = { version: 1 as const, id: workerId("worker-1"), status: "waiting" as const, turn: 1, lastCommandSeq: 1, lastEventSeq: 1 };
  ...
  const ctx = { hasUI: true, mode: "tui", ui: { select, input: vi.fn(async () => message), confirm: vi.fn(async () => true), notify: vi.fn() } };
  return { manager, ctx };
}

describe("subagent controls", () => {
  it.each([["Send follow-up", "send"], ["Steer", "steer"]] as const)("dispatches %s through Manager", async (action, method) => {
  ...
  it("reports stale or missing workers without throwing", async () => {
    ...
    await openSubagentsControl(manager as any, ctx as any);
    expect(ctx.ui.notify).toHaveBeenCalledWith(expect.stringContaining("worker missing"), "error");
  });
