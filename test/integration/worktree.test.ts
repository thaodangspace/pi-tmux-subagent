...
describe("worktree isolation", () => {
  it("creates metadata and refuses destructive dirty cleanup", async () => {
    const root = await mkdtemp(join(tmpdir(), "pi-sa-git-"));
    roots.push(root);
    ...
    const workspace = await adapter.prepare(root, "worker11");
    expect(workspace).toMatchObject({
      branch: "pi-sa/worker11",
      mode: "worktree",
    });
    expect(await readFile(join(workspace.worktree!, "file.txt"), "utf8")).toBe(
      "base\n",
    );
    await writeFile(join(workspace.worktree!, "file.txt"), "changed\n");
    expect((await adapter.inspect(workspace)).changedFiles).toEqual([
      "file.txt",
    ]);
    await expect(adapter.cleanup(workspace)).rejects.toThrow(/dirty/);
  });
});
