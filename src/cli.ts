...
export async function main(argv = process.argv.slice(2)): Promise<number> {
  ...
  const manager = new Manager({
    ...(process.env.PI_TMUX_REGISTRY
      ? { store: new ProtocolStore(process.env.PI_TMUX_REGISTRY) }
      : {}),
  });
  const id = argv[1];
  try {
