...
export class Recovery {
  constructor(private readonly store: ProtocolStore, private readonly tmux: TmuxAdapter, private readonly staleMs = 10_000) {}
  ...
  async recover(value: string): Promise<WorkerState> {
    const id = workerId(value); const [meta, cached, events] = await Promise.all([this.store.readMeta(id), this.store.readState(id), this.store.readLog<WorkerEvent>(id, "events")]);
    ...
    if (!TERMINAL.has(state.status)) {
      ...
      if (!fresh || !session || !alive(meta.runnerPid) || !alive(meta.piPid)) {
        ...
        const event = await this.store.appendEvent(id, { type: "orphaned", data: reasons }); state = reduceEvents(state, [event]);
      }
    }
