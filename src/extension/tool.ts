...
const subagentSchema = Type.Object({
  action: StringEnum(["spawn", "send", "steer", "status", "result", "stop", "delete", "list"] as const),
  id: Type.Optional(Type.String()), task: Type.Optional(Type.String()), message: Type.Optional(Type.String()),
  name: Type.Optional(Type.String()), agent: Type.Optional(Type.String()), provider: Type.Optional(Type.String()), model: Type.Optional(Type.String()),
  thinking: Type.Optional(Type.String()), workspace: Type.Optional(StringEnum(["current", "worktree"] as const)),
});
export interface SubagentInput { action: "spawn" | "send" | "steer" | "status" | "result" | "stop" | "delete" | "list"; id?: string; task?: string; message?: string; name?: string; agent?: string; provider?: string; model?: string; thinking?: string; workspace?: "current" | "worktree" }
...
export function registerSubagentTool(pi: ExtensionAPI, manager = new Manager()): void {
  pi.registerTool({
    ...
    async execute(_id, input, signal, onUpdate, ctx) {
      ...
      let value: unknown;
      if (input.action === "spawn") value = await manager.spawn(await resolveLaunch(ctx.cwd, requireValue(input.task, "task"), input.agent, { ...(input.name ? { name: input.name } : {}), ...(input.provider ? { provider: input.provider } : {}), ...(input.model ? { model: input.model } : {}), ...(input.thinking ? { thinking: input.thinking } : {}), ...(input.workspace ? { workspace: input.workspace } : {}) }), ctx.cwd);
      ...
      else if (input.action === "delete") { const id = requireValue(input.id, "id"); await manager.delete(id); value = { id, deleted: true }; }
