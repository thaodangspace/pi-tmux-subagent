...
export async function openSubagentsControl(manager: Manager, ctx: ExtensionCommandContext): Promise<void> {
  ...
  try {
    ...
    if (!selected) { ctx.ui.notify("Worker is no longer available; refresh and try again", "warning"); return; }
    const state = await manager.status(selected.id);
    const [meta, result] = await Promise.all([manager.store.readMeta(selected.id), manager.result(selected.id)]);
    const detail = [
    ...
    if (action === "Send follow-up" || action === "Steer") {
      ...
      if (!message) return;
      if (action === "Send follow-up") await manager.send(selected.id, message); else await manager.steer(selected.id, message);
      ctx.ui.notify(`${action} queued for ${selected.id}`, "info");
    } else if (action === "Stop") {
      if (await ctx.ui.confirm("Stop subagent?", `Queue a graceful stop for ${selected.id}?`)) {
...
function bounded(text: string): string { const flat = text.replace(/\s+/g, " ").trim(); return flat.length > 160 ? `${flat.slice(0, 159)}…` : flat; }
