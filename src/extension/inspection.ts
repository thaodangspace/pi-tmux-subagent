...
export async function inspectWorker(manager: Manager, id: string, activityLimit = 8): Promise<string> {
  ...
  const input = { state, meta, events, commands, ...(result ? { result } : {}) };
  ...
  const lines = [
    ...
    meta.workspace?.worktree ? `worktree: ${meta.workspace.worktree}` : undefined,
    meta.workspace?.changedFiles?.length ? `changed: ${meta.workspace.changedFiles.join(", ")}` : undefined,
    view.latestActivity ? `latest: ${view.latestActivity}` : undefined,
    "",
    "Recent activity:",
    ...recentActivity(input, activityLimit).map((item) => `${item.at} [${item.kind}] ${item.text}`),
  ];
  if (result) lines.push("", `Latest result (${result.completedAt}):`, bounded(result.text, 2_000));
